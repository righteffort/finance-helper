  ...
  async evaluate(_fn: Function, ...args: any[]): Promise<any> {
    ...
    if (typeof url === "string") {
      if (url.endsWith("ftgw/digital/portfolio/api/graphql?ref_at=portsum")) {
        return JSON.parse(
          readFileSync(
            join(TESTDATA, `mock_get_accounts_response_${this.suffix}.json`),
            "utf8",
          ),
        );
      } else if (
        url.endsWith("ftgw/digital/webactivity/api/graphql?ref_at=activity")
      ) {
        const { body } = args[1];
        const bodyObj = JSON.parse(body) as GetTransactionsReqModel;
        expect(bodyObj.variables.searchCriteriaDetail.txnFromDate).toBe(
          this.expectedStart,
        );
        expect(bodyObj.variables.searchCriteriaDetail.txnToDate).toBe(
          this.expectedEnd,
        );
        return JSON.parse(
          readFileSync(
            join(
              TESTDATA,
              `mock_get_transactions_response_${this.suffix}.json`,
            ),
            "utf8",
          ),
        );
  ...
  for (const suffix of getSuffixes()) {
    it(`should get transactions [${suffix}]`, async () => {
      const config = JSON.parse(
        readFileSync(join(TESTDATA, `config_${suffix}.json`), "utf8"),
      );
      ...
      const expected = JSON.parse(
        readFileSync(
          join(TESTDATA, `get_transactions_expected_${suffix}.json`),
          "utf8",
        ),
      );

...
export class Fidelity {
  ...
  async getTransactions(
    ...
    const dateMessage = [Fidelity.checkDate(start), Fidelity.checkDate(end)]
      .filter((m) => m.length > 0)
      .join("; ");
    ...
    const accts = Array.from(acctDict.values()).filter(
    ...
    try {
      ...
      throw new FidelityError(
        `Failed to parse get_transactions response: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  static getAccountsOptions(): object {
    return JSON.parse(
      fs.readFileSync(
        path.join(packageDir, "../fidelity-templates/getAccountsOptions.json"),
        "utf8",
      ),
    );
  }
  ...
  static getTransactionsOptions(
    ...
    const context = {
      body: JSON.stringify(
        JSON.stringify(Fidelity.getTransactionsBody(accounts, start, end)),
      ),
    };
    return JSON.parse(
      Mustache.render(Fidelity.getTransactionsOptionsTemplate(), context),
    );
  }
   ...
   */
  static printFetchTransactionsOptions(options: {
    body: string;
    [key: string]: unknown;
  }): string {
  ...
  private async fetch(url: string, options: object): Promise<object> {
    ...
    if (resp.status !== 200) {
      throw new FidelityError(
        `get_transactions fetch failed, code: ${resp.status}, text: ${resp.text}`,
      );
    }
    return resp.json;
  }
  ...
  private async fetchAccounts(): Promise<Map<string, Account>> {
    ...
    try {
      ...
      throw new FidelityError(
        `Failed to parse get_accounts response: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }

  private static getTransactionsBody(
    accounts: Account[],
    start: Date,
    end: Date,
  ): GetTransactionsReqModel {
    ...
    const body = Mustache.render(
      Fidelity.getTransactionsBodyTemplate(),
      context,
    );
  ...
  private static getTransactionsBodyTemplate(): string {
    return fs.readFileSync(
      path.join(
        packageDir,
        "../fidelity-templates/getTransactionsBody.json.mustache",
      ),
      "utf8",
    );
  }
  ...
  private static getTransactionsOptionsTemplate(): string {
    return fs.readFileSync(
      path.join(
        packageDir,
        "../fidelity-templates/getTransactionsOptions.json.mustache",
      ),
      "utf8",
    );
  }

  private static historyEntryToTransaction(
    ...
    if (amount === undefined && !pending) {
      throw new FidelityError(
        `Unexpected amount string ${h.amount} in history entry ${JSON.stringify(h)}`,
      );
    }
    const cashBalance = Fidelity.amountStringToFloat(h.cashBalance);
    if (cashBalance === undefined && h.cashBalance !== undefined && !pending) {
      throw new FidelityError(
        `Unexpected cashBalance string ${h.cashBalance} in history entry ${JSON.stringify(h)}`,
      );
    }
    return {
      acctNum: h.acctNum,
      ...
      ...(h.orderNumber !== undefined &&
        h.orderNumber != null && { orderNumber: h.orderNumber }),
      pending,
    };
  }

  private static amountStringToFloat(
    s: string | undefined,
    allowDashDash = false,
  ): number | undefined {
    if (s === undefined) {
